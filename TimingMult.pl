...
#################--??????????????????????--################
	...
	{	
		if ($j == 0)
		{
			...
			{
				if($bitarray[$i][$k][$j]->in == 0)# in parametresi gereksiz bitarray sort edildi�i i�in (FA*3+HA*2) tane eleamandan sonrakilerin sadece in parametresi 0 olur
				{
					$bitarray[$i+1][$unusedbitlocation][$j] = $bitarray[$i][$k][$j];#kopyalamak yerine new ile yeni olu�turmak gerekebilir
					$unusedbitlocation++;
				}
			}
...
{
	open Mult, ">Compressor.v";
		print Mult "module Compressor (\n";
		for ($j=0;$j<$rownum;$j++)
		{
			for ($k=0;$k<$length;$k++)
			{
